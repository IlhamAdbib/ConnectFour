...
public class ConnectFourView implements ConnectFourObserver {
  ...
  private ConnectFourController controller;//interaction w ith game logic
  ...
  private JButton[][] boardCells;//ARRAY OF JBTN to represent the slots on the board
  ...
  public ConnectFourView(ConnectFourController controller) {
    this.board = new BoardBuilder().buildFromRow(GameConfig.ROW).buildFromCol(GameConfig.COL).build();//set a new board using builder DP
    this.controller = controller;
    this.controller.registerObserver(this);
    this.refresh(); //ensure that the view ids ready to display changes
  }
   ...
   * @param col
   * @param color
   *  Called during gameplay to visually mark players' moves on the board and highlight winning sequences.
   */
  ...
  public void hideView() {//Makes the main game window invisible.
   ...
   *
   */
  private class BoardButtonActionListener implements ActionListener {/*Purpose: Handles player interactions with the board.
Implementation: Detects button clicks, identifies the column selected by the player, and communicates this move to the controller. Also handles errors like column overflow or invalid moves (e.g., clicking during another player's turn).*/
    @Override
  ...
  private class SelectedPlayerListener implements ActionListener{//UPDATES PLAYER TYPES BASED ON SELECT . tells the choices to the game model via controller
...
//resets the game to its initail state useful for restarting game after a game over
  private void refresh() {
    ...
    this.setboardCells();
    frameMain = new JFrame("Connect Four ");
